...
class TestSanity(unittest.TestCase):
    ...
    def test_scale_modularity_floor_on_karate(self):
        q = modularity(self.graph, as_communities(pymocd.scale(self.graph)))
    ...
    def test_metrics_perfect_self_agreement(self):
        partition = pymocd.scale(self.graph)
    ...
    def test_isolated_node_gets_minus_one(self):
        ...
        self.assertEqual(pymocd.scale(self.graph)[99], -1)

    def test_single_community_graph(self):
        partition = pymocd.scale(nx.complete_graph(6))
        self.assertEqual(set(partition), set(range(6)))

    def test_empty_graph(self):
        self.assertEqual(pymocd.scale(nx.Graph()), {})
