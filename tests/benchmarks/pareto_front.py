...
solutions = [
    (comm, intra, inter, 1 - intra - inter) for comm, (intra, inter) in pareto_front
]
...
metrics = [
    evaluate_communities(G, comm, ground_truth, convert=False) for comm, *_ in solutions
]
...
best_mod, best_nmi, best_ami = (
    best_metrics["modularity"],
    best_metrics["nmi"],
    best_metrics["ami"],
)
...
plt.figure(figsize=(4, 3))
plt.scatter(
    num_coms, q_vals, s=50, alpha=0.7, edgecolors="black", label="All solutions"
)
plt.scatter(
    best_num_com,
    best_q,
