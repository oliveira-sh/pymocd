//! Reference-point association and niche-preserving selection.
...
use super::normalize::normalize_objectives;

const DEGENERATE_REF_NORM2: f64 = 1e-30;
...
/// `>= k_chosen`, i.e. members of the splitting front — filling `need` slots.
pub fn niche_select(
    combined: &[Individual],
    st_indices: &[usize],
    k_chosen: usize,
    ...
    let normalized = normalize_objectives(combined, st_indices, m);
...
/// Distance from `point` to the origin-anchored line along `ref_dir`, whose
/// squared norm `rnorm2` the caller precomputed.
fn perp_distance(point: &[f64], ref_dir: &[f64], rnorm2: f64) -> f64 {
    if rnorm2 < DEGENERATE_REF_NORM2 {
