//! Environment selection: which individuals survive to the next generation.
...
/// Survivor indices: whole fronts by ascending rank, the overflowing one
/// truncated by descending crowding distance. Returns `min(keep, len)` of them.
