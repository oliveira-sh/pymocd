//! Fast non-dominated sorting: assigns a Pareto rank to every population member.
...
/// Assigns Pareto ranks in place, front 1 being the non-dominated set (Deb et al. 2002).
pub fn fast_non_dominated_sort(population: &mut [Individual]) {
    ...
    while !fronts[front_idx].is_empty() {
        ...
        let mut next_front: Vec<usize> = current_front
            .par_iter()
            .fold(Vec::new, |mut acc, &i| {
                ...
                for &j in &dominated_data[range.start..range.end] {
                    ...
                    // fetch_sub returns the old count, so prev == 1 is the last dominator gone
                    if prev == 1 {
                        acc.push(j);
                    }
                }
        ...
        // whichever thread lands the last decrement owns the push, so sort the race out
