//! Shared NSGA-III core (Deb & Jain, IEEE TEC 18(4):577–601, 2014) over the
//! same `Individual` as `nsga2`; only survivor selection differs — crowding
//! distance is replaced by reference-point niching. Objective count `M` is
//! inferred from `Individual::objectives.len()`, so the engine is
//! objective-count generic.
...
pub fn evolve<E>(
    ...
    let mut pop: Vec<Individual> = operators::generate_population(graph, pop_size)
        ...
        .collect();
    evaluate(&mut pop)?;
    // Ranks for generation-0 mating selection: the reused nsga2 tournament
    // reads `rank`.
...
fn environmental_selection(
    ...
    // (all kept), `last_front` = Fl (the splitting front, niched).
